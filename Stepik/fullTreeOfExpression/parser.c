...
int parse_addition(Expression *expr, Token *buf);

int parse_primary(Expression *expr, Token *buf, bool sign)
{
    Expression tmp = NULL;
    ...
    if ((!sign) && buf->type == OPERATOR && (buf->data.operator_name == '+' || buf->data.operator_name == '-')) {
        ...
        token_copy(&s, buf);
        token_next(buf);
        err = parse_primary(&tmp, buf, true);
        if (err) {
...
int parse_power(Expression *expr, Token *buf)
{
    Expression tmp = NULL;
    int err = PARSER_SUCCESS;
    if (buf->type == FINAL) {
    ...
    err = parse_primary(&tmp, buf, false);
    if (err) {
        expression_destroy(&tmp);
        return PARSER_ERROR;
    }
...
int parse_multiplication(Expression *expr, Token *buf)
{
    Expression tmp = NULL;
    int err = PARSER_SUCCESS;
...
int parse_addition(Expression *expr, Token *buf)
{
    ...
    if (buf->type == OPERATOR) {
        if (priority(buf->data.operator_name) == ADD) {
            ...
            token_next(buf);
            err = parse_multiplication(&tmp, buf);
            if (err) {
                token_free(&t);
                expression_destroy(&tmp);
                return PARSER_ERROR;
        ...
        err = parse_multiplication(expr, buf);
        if (err) {
            return PARSER_ERROR;
        }
    }
...
int parse(Expression *expr)
{
    ...
    int err = parse_addition(expr, &buf);
